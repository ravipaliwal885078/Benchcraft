...
def get_comprehensive_dashboard():
    ...
    try:
        ...
        # ===== INSIGHTS (Generated by AI Agent) =====
        try:
            from agents.insights_agent import InsightsAgent
            insights_agent = InsightsAgent()
            insights = insights_agent.generate_insights()
        except Exception as e:
            # Fallback to static insights if AI agent fails
            import traceback
            print(f"Warning: Insights agent failed, using fallback: {e}")
...
def chat():
    """Chat agent endpoint for conversational queries with history"""
    try:
        ...
        response = chat_agent.query(question, conversation_history=conversation_history, use_web_search=use_web_search)
        
        return jsonify({
            'answer': response['answer'],
            'sources': response.get('sources', []),
            'data': response.get('data', {}),
            'web_context': response.get('web_context', False),
